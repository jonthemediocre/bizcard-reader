...
export const extractBusinessCardData = async (imageBase64: string): Promise<BusinessCard> => {
  try {
    ...
    const extractedText = (textResponse.data as any).choices[0].message.content;
    ...
    const parsedCard: BusinessCard = JSON.parse((parseResponse.data as any).choices[0].message.content);
