...
export class EnterpriseVault {
  ...
  async getSecret(key: string): Promise<VaultSecret | null> {
    try {
      ...
      // Fetch from MCP Vault using dynamic import to avoid TypeScript issues
      const vaultManager = await import('../../mcp/vault-manager');
      const secretResult = await vaultManager.retrieveSecret(key);

      if (!secretResult || !secretResult.value) {
        return null;
      }
      ...
      const secret: VaultSecret = {
        ...
        category: secretResult.category || 'other',
        tags: secretResult.tags || [],
        metadata: {
          key: key,
          category: secretResult.category || 'other',
          tags: secretResult.tags || [],
  ...
  async setSecret(
    ...
    try {
      // Store in MCP Vault
      const vaultManager = await import('../../mcp/vault-manager');
      const success = await vaultManager.storeSecret(key, value, category, tags);
      ...
      if (success) {
        ...
        this.cache.set(this.getCacheKey(key), cacheEntry);
      }
  ...
  async deleteSecret(key: string): Promise<boolean> {
    try {
      const vaultManager = await import('../../mcp/vault-manager');
      const success = await vaultManager.deleteSecret(key);
  ...
  async listSecrets(category?: 'api_key' | 'password' | 'token' | 'certificate' | 'other', tags?: string[]): Promise<string[]> {
    try {
      const vaultManager = await import('../../mcp/vault-manager');
      const secretsList = await vaultManager.listSecrets(category, tags);
      
      // Extract just the keys
      return secretsList.map((secret: any) => secret.key || secret.name || secret);
    } catch (error) {
  ...
  async rotateSecret(key: string, newValue: string): Promise<boolean> {
    try {
      const vaultManager = await import('../../mcp/vault-manager');
      const success = await vaultManager.rotateSecret(key, newValue);
      ...
      if (success) {
        ...
        if (existing) {
          existing.secret.value = newValue;
          existing.secret.metadata.last_rotated = new Date();
          existing.expires = Date.now() + this.config.cache_ttl;
        }
      }
